...
buddys_alloc(size_t size){
    ...
    uintptr_t res = 0;
    panic_on(size < PAGE_SIZE,"size is too small");
    panic_on(size > MAXSIZE, "size is too big");
    panic_on(size % 2 != 0,"size is invalid");
...
buddys_free(Chunk *chunk){
  ...
  while (idx < buddys_size - 1)
  {
      ...
      if ((CHUNKS_GET_IDX(another_chunk) != idx) || (CHUNKS_GET_STATUS(another_chunk) != CHUNKS_STATUS_UNUSE) || (CHUNKS_GET_FLAG(another_chunk) != CHUNKS_FLAG_BUDDY)){
        break;
      }
...
slabs_alloc(size_t size){
    ...
    panic_on(size >= PAGE_SIZE, "size is too big");
    ...
    do
    {
        ...
        {
          if (lock_try_acquire(&(head->un.lock)) == PMMLOCKED){
            goto PREPARE_BEFORE_NEXT;
          }
        }
    ...
    if ((res = (uintptr_t)buddys_alloc(PAGE_SIZE)) == 0){
        return NULL;
    }
    panic_on(BUDDY_IDX2CHUNK_SIZE(CHUNKS_GET_IDX(res)) != PAGE_SIZE, "error idx");
    panic_on(CHUNKS_GET_STATUS(res) != CHUNKS_STATUS_INUSE,"error status");
    panic_on(CHUNKS_GET_FLAG(res) != CHUNKS_FLAG_BUDDY, "error flag");
    ...
    for (uintptr_t iter = gap; iter < PAGE_SIZE; iter += gap){
...
static void *kalloc(size_t size) {
  ...
  void *res = size_align < PAGE_SIZE ? slabs_alloc(size_align) : buddys_alloc(size_align);
...
static void pmm_init() {
   ...
   * 其元素个数不超过     (heap.end - heap.start + PAGE_SIZE - 1) / PAGE_SIZE
   * 
   * 对于buddys结构来说，， 其包含元素的个数就是log_ceil(MAXSIZE / PAGE_SIZE) + 1
   * 
   * 对于slabs结构来说，其是一个二维数组，slabs指向一个cpu_count()数目个元素的数组
   * 其元素是一个log_ceil(PAGE_SIZE / 2 / MINSIZE) + 1个CHUNK结构的数组
   */
  chunks = (uintptr_t*)heap.start;
  chunks_size = (((uintptr_t)heap.end) - ((uintptr_t)heap.start) + PAGE_SIZE - 1) / PAGE_SIZE;
  ...
  //首先测试其log_ceil函数
    ...
    for(int i = 0; i < 0x40; ++i) {
      ...
      ptr = chunks_base +  (rand() % chunks_size) * PAGE_SIZE;
      ...
      panic_on(chunks[(ptr - chunks_base) / PAGE_SIZE] != (((flag << (CHUNKS_IDX_SIZE + CHUNKS_STATUS_SIZE)) | (status << (CHUNKS_IDX_SIZE)) | (idx))), "error CHUNKS_SET_");
    }


    //将chunks所有的值填充为CHUNKS_FLAG_SLAB | CHUNKS_STATUS_INUSE | CHUNKS_IDX_MASK，方便提前检查错误
    for(uintptr_t iter = chunks_base; iter < (uintptr_t)(heap.end); iter += PAGE_SIZE) {
  ...
  buddys_size = log_ceil(MAXSIZE / PAGE_SIZE) + 1;
  ...
  slabs_size = log_ceil(PAGE_SIZE / 2 / MINSIZE) + 1;
              ...
              if(counts[mode]) {
                ...
                switch(mode) {
                  case 0:
                    ...
                    break;
                  case 1:
                    array_size[size] = PAGE_SIZE * (1 + (rand() % (BOUNDARY2 / PAGE_SIZE)));
                    break;
                  case 2:
